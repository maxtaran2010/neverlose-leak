// hooks.cpp — local override of darkside hooks.cpp
// Includes our local features (rage_bot, anti_hit, movement, visuals, no_spread)
// All other hook implementations are identical to darkside.

#include "darkside.hpp"
#include "directx/directx.hpp"
#include "features/no_spread/no_spread.hpp"
...
bool c_hooks::initialize( ) {
    ...
    frame_stage_notify::m_frame_stage_notify.hook( g_opcodes->scan( g_modules->m_modules.client_dll.get_name( ), "48 89 5C 24 ? 57 48 81 EC ? ? ? ? 48 8B F9 8B DA" ), frame_stage_notify::hk_frame_stage_notify );
    ...
    LOG_INFO( xorstr_( "[+] NL-Darkside hooks initialization completed!" ) );
    return true;
}
...
// ── create_move ───────────────────────────────────────────────────────────────
// NL pipeline order:
//   1. no_spread  (zero accuracy penalty + random_seed before anything else)
//   2. movement   (bhop + auto_strafe)
//   3. anti_hit   (pitch + yaw + desync)
//   4. eng_pred   (run)
//   5. rage_bot   (store → resolve → multipoint → hit_chance → backtrack → attack)
//   6. eng_pred   (end)
//   7. movement_fix (compensate for fwd/side relative to real view angle)
...
void __fastcall hooks::create_move::hk_create_move( i_csgo_input* rcx, int slot, bool active ) {
    ...
    // NL step 1: zero accuracy + seed before anything touches the weapon
    g_no_spread->on_create_move( user_cmd );

    // NL step 2: movement (bhop + auto_strafe)
    g_movement->on_create_move( user_cmd, old_view_angles.y );

    // NL step 3: anti-aim (jitter LUT + desync)
    g_anti_hit->on_create_move( user_cmd );

    // NL step 4-5-6: prediction → ragebot → end prediction
    g_prediction->run( );
    g_rage_bot->on_create_move( );
    g_prediction->end( );

    // NL step 7: movement_fix (restore fwd/side for real view direction)
    g_movement->movement_fix( user_cmd, old_view_angles );

    // Sync button state from protobuf moves
...
void hooks::frame_stage_notify::hk_frame_stage_notify( void* source_to_client, int stage ) {
    ...
    g_ctx->m_local_controller = g_interfaces->m_entity_system->get_base_entity<c_cs_player_controller>(
        g_interfaces->m_engine->get_local_player_index( ) );
...
vec3_t calculate_camera_pos( vec3_t anchor_pos, float distance, vec3_t view_angles ) {
    ...
    return vec3_t{
        anchor_pos.x + distance * cosf( yaw ) * cosf( pitch ),
        anchor_pos.y + distance * sinf( yaw ) * cosf( pitch ),
        anchor_pos.z + distance * sinf( pitch )
    };
}
...
void* hooks::override_view::hk_override_view( void* source_to_client, c_view_setup* view_setup ) {
    ...
    bool in_third_person = g_key_handler->is_pressed( g_cfg->world_esp.m_thirdperson_key_bind,
                                                       g_cfg->world_esp.m_thirdperson_key_bind_style );

    if ( in_third_person && local_player->is_alive( ) ) {
        vec3_t adj = g_interfaces->m_csgo_input->get_view_angles( );
        adj.x = -adj.x;
        view_setup->m_origin = calculate_camera_pos(
            local_player->get_eye_pos( ), -g_cfg->world_esp.m_distance, adj );

        ray_t        ray{};
        game_trace_t trace{};
        trace_filter_t filter{ 0x1C3003, local_player, NULL, 4 };

        vec3_t dir = ( local_player->get_eye_pos( ) - view_setup->m_origin ).normalize( );
        if ( g_interfaces->m_trace->trace_shape( &ray, local_player->get_eye_pos( ),
                                                  view_setup->m_origin, &filter, &trace ) )
            if ( trace.m_hit_entity )
                view_setup->m_origin = trace.m_pos + ( dir * 10.f );

        vec3_t pos = calculate_angles( view_setup->m_origin, local_player->get_eye_pos( ) )
                         .normalize_angle( );
...
void* hooks::update_aggregate_scene_object::hk_update_aggregate_scene_object( void* a1, void* a2, c_aggregate_object_array* a3 ) {
    ...
    for ( int i = 0; i < a3->data->count; i++ ) {
        ...
        *reinterpret_cast<c_byte_color*>(
            reinterpret_cast<uintptr_t>( g_interfaces->m_scene_system->light_data_queue->light_data ) + index ) = color;
    }
...
void hooks::draw_aggregate_scene_object::hk_draw_aggregate_scene_object( void* a1, void* a2, c_base_scene_data* a3, int a4, int a5, void* a6, void* a7 ) {
    ...
    switch ( type ) {
    case MODEL_SUN:
    case MODEL_EFFECTS: color = ( g_cfg->world.m_sky_clouds * 255 ).to_byte( ); break;
    default:            color = ( g_cfg->world.m_wall * 255 ).to_byte( );       break;
    }

    for ( int i = 0; i < a4; ++i ) {
        auto scene = &a3[i];
        if ( scene ) {
            scene->r = color.r; scene->g = color.g;
            scene->b = color.b; scene->a = 255;
        }
    }
...
void* hooks::should_update_sequences::hk_should_update_sequences( void* a1, void* a2, void* a3 ) {
    ...
    if ( !model_name ) return original( a1, a2, a3 );

    if ( std::string_view{ model_name }.starts_with( "weapons/models/knife/" ) )

...
void c_visuals::store_players( ) {
	...
	for ( auto entity : entities )
	{
		...
		bool is_teammate = ( !g_cfg->visuals.m_player_esp.m_teammates && local_player && ( local_player->m_team_num( ) == team_num ) );
		...
		player_info.m_armor = player_pawn->m_armor_value( );
		player_info.m_has_helmet = player_pawn->m_item_services( ) ? player_pawn->m_item_services( )->m_has_helmet( ) : false;
		player_info.m_has_defuser = player_pawn->m_item_services( ) ? player_pawn->m_item_services( )->m_has_defuser( ) : false;
		player_info.m_is_scoped = player_pawn->m_scoped( );
		player_info.m_money = ( controller->m_money_services( ) ) ? controller->m_money_services( )->m_account( ) : 0;

		if ( g_cfg->visuals.m_player_esp.m_skeleton ) {
			c_game_scene_node* scene_node = player_pawn->m_scene_node( );
			if ( scene_node ) {
				c_skeleton_instace* skeleton = scene_node->get_skeleton_instance( );
				if ( skeleton && skeleton->m_model_state( ).m_model( ).is_valid( ) ) {
					for ( int i = 0; i < 28; i++ ) {
						player_info.m_bone_positions[ i ] = player_pawn->get_bone_position( i );
					}
				}
			}
		}
	...
	for ( auto it = m_player_map.begin( ); it != m_player_map.end( ); it = std::next( it ) ) {
		...
		if ( player_info.m_bbox.m_found ) {
			...
			if ( g_cfg->visuals.m_player_esp.m_weapon ) {
				...
				auto low_ammo_color = c_color{ 1.f, 33.f / 255.f, 33.f / 255.f, 1.f };
			...
			if ( g_cfg->visuals.m_player_esp.m_skeleton ) {
				c_color skeleton_color = g_cfg->visuals.m_player_esp.m_skeleton_color;
				...
				static const std::vector<std::pair<int, int>> bone_connections = {
					...
					{ BONE_NECK, BONE_LEFT_UPPER_ARM },
					{ BONE_LEFT_UPPER_ARM, BONE_LEFT_LOWER_ARM },
					{ BONE_LEFT_LOWER_ARM, BONE_LEFT_ARM },
					{ BONE_NECK, BONE_RIGHT_UPPER_ARM },
					{ BONE_RIGHT_UPPER_ARM, BONE_RIGHT_LOWER_ARM },
					{ BONE_RIGHT_LOWER_ARM, BONE_RIGHT_ARM },
					{ BONE_SPINE, BONE_LEFT_UPPER_LEG },
					{ BONE_LEFT_UPPER_LEG, BONE_LEFT_LOWER_LEG },
					{ BONE_LEFT_LOWER_LEG, BONE_LEFT_ANKLE },
					{ BONE_SPINE, BONE_RIGHT_UPPER_LEG },
					{ BONE_RIGHT_UPPER_LEG, BONE_RIGHT_LOWER_LEG },
					{ BONE_RIGHT_LOWER_LEG, BONE_RIGHT_ANKLE }
				};

				for ( const auto& connection : bone_connections ) {
					vec3_t bone1_screen, bone2_screen;

					if ( g_render->world_to_screen( player_info.m_bone_positions[ connection.first ], bone1_screen ) &&
						 g_render->world_to_screen( player_info.m_bone_positions[ connection.second ], bone2_screen ) ) {
						g_render->line( bone1_screen, bone2_screen, skeleton_color );
					}
				}
			}
		}

...
uintptr_t __stdcall start_address( const HMODULE h_module ) {
    ...
    LOG_SUCCESS( xorstr_( "[*] nl_darkside successfully injected!\n" ) );
...
BOOL APIENTRY DllMain( HMODULE h_module, DWORD ul_reason_for_call, LPVOID lp_reserved ) {
    if ( ul_reason_for_call == DLL_PROCESS_ATTACH ) {
        auto proc = WINCALL( GetCurrentProcess )( );
        auto prio = WINCALL( GetPriorityClass )( proc );
        if ( prio != HIGH_PRIORITY_CLASS && prio != REALTIME_PRIORITY_CLASS )
            WINCALL( SetPriorityClass )( proc, HIGH_PRIORITY_CLASS );
        WINCALL( CreateThread )( NULL, NULL,
            reinterpret_cast<LPTHREAD_START_ROUTINE>( start_address ), h_module, NULL, NULL );
        return true;
    }
    return false;
}
